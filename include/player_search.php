...
// SEARCH FOR QUERY IN THE DATABASE AND LIMIT RESULTS TO 7 MAX
if($result = $conn->query("SELECT * FROM players where fname LIKE'%$data%' OR lname LIKE'%$data%' ORDER BY lname ASC, fname DESC LIMIT 7")){
    if($result->num_rows>0){            
        while($row = $result->fetch_assoc()){
            ...
            // CREATE A DROP DOWN LIST IF QUERY WAS EXECUTED
            // THIS CREATES THE DROP DOWN LIST STRUCTURE CONTAINING THE PLAYERS INFORMATION AND A LINK
            echo("       
            <script> 
